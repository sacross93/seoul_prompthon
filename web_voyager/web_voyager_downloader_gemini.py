...
llm = ChatGoogleGenerativeAI(model="gemini-1.5-pro",
                             google_api_key="AIzaSyDq4RUxqzoOJA_8U9PSCE2ai-GJNkxgWSg",
                             verbose=True,
                             )
...
async def mark_page(page):  # 마킹하고, 스크린샷 찍고
    ...
    for _ in range(10):  # 10번 시도.
        ...
        bboxes = []  # 모든 시도가 실패해도 bboxes가 정의되도록 함
...
async def annotate(state):  # 나중에 retry를 다시 사용해볼 것
...
prompt = ChatPromptTemplate(
    ...
    messages=[MessagesPlaceholder(variable_name='scratchpad', optional=True),
              HumanMessagePromptTemplate(prompt=[
                  ...
                  ImagePromptTemplate(input_variables=['img'], template={
...
def update_scratchpad(state: AgentState):
    """툴이 호출된 후, 에이전트가 이전 단계를 인식할 수 있도록 scratchpad를 업데이트합니다."""
    old = state.get("scratchpad")
    if old:
        txt = old[0].content if isinstance(old[0], SystemMessage) else old[0]
...
async def run_browser():
    async with async_playwright() as p:
        ...
        await call_agent("Find you WebVoyager paper (on arxiv) file on google. Download that PDF file and finish action  ", page)
